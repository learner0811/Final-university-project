...
import hn46.sa.mapper.RoomOwnerMapper;
...
public class RoomOwnerDao implements BaseDao<RoomOwner> {
	...
	public RoomOwner save(final RoomOwner roomOwner) {
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
				String sql = "insert into room_owner (name, gender, age, native, interests, civil_status, smoker, ocupation, iduser, idroom, idroommate_criteria, phone, email,room_number,room_available) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				ps.setInt(14, landlord.getRoomNumber());
				ps.setInt(15, landlord.getAvailable());
				return ps;
			}
		}, keyHolder);
	...
	public RoomOwner update(RoomOwner roomOwner) {
		...
		Object[] params = { roomOwner.getName(), roomOwner.getAge(), roomOwner.getStrNative(), roomOwner.getInterests(),
				...
				roomOwner.getRoommateCriteria().getIdRoommateCriteria(), roomOwner.getPhone(), roomOwner.getEmail(), roomOwner.getChange(), landlord.getRoomNumber(), landlord.getAvailable(), roomOwner.getType(), roomOwner.getIdRoomOwner() };
		int[] types = { Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.INTEGER,
				Types.VARCHAR, Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER,Types.INTEGER };
		System.out.println(roomOwner.getIdUser());
		String updateSql = "update room_owner set name = ?, age = ?, native = ?, interests = ?, civil_status = ?, smoker = ?, ocupation = ?, status = ?, iduser = ?, idroom = ?, idroommate_criteria = ?, phone = ?, email = ?, is_change = ?, room_number = ?, room_available = ?, type = ? where idroomowner = ?";
	...
	public List<RoomOwner> findAll() {
		String sql = "select * from room_owner where status = 1";
