...
import sa.admin.mapper.RoomOwnerMapper;
...
public class RoomOwnerDao implements BaseDao<RoomOwner> {
	...
	public RoomOwner update(RoomOwner roomOwner) {
		Object[] params = { roomOwner.getName(), roomOwner.getAge(), roomOwner.getStrNative(), roomOwner.getInterests(),
				...
				roomOwner.getRoommateCriteria().getIdRoommateCriteria(), roomOwner.getPhone(), roomOwner.getEmail(), roomOwner.getChange(), roomOwner.getIdRoomOwner() };
		...
		String updateSql = "update room_owner set name = ?, age = ?, native = ?, interests = ?, civil_status = ?, smoker = ?, ocupation = ?, status = ?, iduser = ?, idroom = ?, idroommate_criteria = ?, phone = ?, email = ?, is_change = ? where idroomowner = ?";
